// Work and Travel RD — Netlify Function para resumen automático de PDF
// v3881: extractor local reforzado, genérico y abierto a sponsors/formats nuevos; no depende de una lista cerrada.
...
function normalize(value = {}) {
    ...
    if (!result.positions.length)
        result.positions = [normalizePosition(result)];
    return result;
}
function base64ToLatin1(base64) {
    try {
        const binary = atob(base64);
        let out = "";
        for (let i = 0; i < binary.length; i++)
            out += String.fromCharCode(binary.charCodeAt(i) & 255);
        return out;
    }
    catch (_) {
        return "";
...
function extractPdfTextHeuristic(base64 = "") {
    ...
    for (const item of printable.slice(0, 5000))
        chunks.push(item);
...
function tidyExtractedValue(value, max = 140) {
    let v = String(value || "")
        .replace(/[•□■◆●]/g, " ")
        .replace(/[\uFFFC\uFFFD]/g, " ")
        .replace(/\s+/g, " ")
...
function extractField(src, field) {
    ...
    for (const pattern of patterns) {
        ...
        if (match) {
            ...
            if (field === "peoplePerRoom" && match[2])
                return tidyExtractedValue(`${match[2]} por habitación`, 80);
...
function parseLocation(src) {
    ...
    if (abbr)
        return { city: tidyExtractedValue(abbr[1], 80), state: US_STATES[abbr[2].toUpperCase()] || abbr[2] };
    const full = src.match(new RegExp(`\\b([A-Z][A-Za-z .'-]{2,60}),\\s*(${STATE_NAMES})\\b`, "i"));
    if (full)
        return { city: tidyExtractedValue(full[1], 80), state: tidyExtractedValue(full[2], 80) };
    return { city: NO, state: NO };
}
function detectCompany(src, fileName = "") {
    const known = [
    ...
    for (const re of known) {
        ...
        if (m)
            return tidyExtractedValue(m[0], 120);
    }
    const direct = extractField(src, "company");
    if (direct !== NO && !/^(Grocery Store|Resort with restaurant|Camp\/Outdoor Work)$/i.test(direct) && !/[--]/.test(direct))
        return direct;
    return direct;
}
function positionObj(base, extra = {}) {
    const cleanBase = { ...(base || {}) };
    delete cleanBase.positions;
    return normalize({ ...cleanBase, ...extra }).positions[0];
}
function detectPositionBlocks(src, base) {
    const positions = [];
    const seen = new Set();
    const add = (obj) => {
        const p = positionObj(base, obj);
        const key = `${p.position}|${p.hourlyPay}|${p.city}|${p.state}`.toLowerCase();
        if (p.position !== NO && !seen.has(key)) {
            seen.add(key);
            positions.push(p);
        }
    };
    ...
    for (const role of gecRoles) {
        ...
        if (m)
            add({ position: role.replace("’", "'"), hourlyPay: m[1] });
    }
    ...
    for (const m of src.matchAll(interRe))
        add({ position: m[1], hourlyPay: m[2], estimatedHours: m[3], overtime: m[4] ? `Overtime wage ${m[4]}` : base.overtime });
    // Greenheart long forms: Job Type -> later Wage.
    const jobTypeRe = /Job Type:\s*([^\n\r:]{2,80})[\s\S]{0,900}?Wage:\s*(\d{1,3}(?:\.\d{2})?\s*\/per\s*hour)/gi;
    for (const m of src.matchAll(jobTypeRe))
        add({ position: m[1], hourlyPay: `$${m[2]}` });
    ...
    for (let i = 0; i < roleMatches.length; i += 1) {
        ...
        if (wage)
            add({ position: m[1], hourlyPay: wage, estimatedHours: hours, overtime: ot ? `Overtime wage ${ot}` : base.overtime });
    }
    // Compact flyers.
    const employment = src.match(/Employment Position:\s*[^A-Za-z0-9]*([^($\n\r]{2,90})\s*\((\$\s*\d{1,3}(?:\.\d{2})?\s*\/hr)\)/i);
    if (employment)
        add({ position: employment[1], hourlyPay: employment[2] });
    const positionHourly = src.match(/Position:\s*([^\n\r]{2,80})[\s\S]{0,450}?Hourly Wage:\s*(\$\s*\d{1,3}(?:\.\d{2})?)/i);
    if (positionHourly)
        add({ position: positionHourly[1], hourlyPay: positionHourly[2] });
    return positions;
}
function keywordSummaryFromText(text = "", fileName = "") {
    ...
    if (found.city === NO && location.city !== NO)
        found.city = location.city;
    if (found.state === NO && location.state !== NO)
        found.state = location.state;
    ...
    if (/Dave\s*&\s*Buster/i.test(flat)) {
        found.company = "Dave & Buster's Atlanta";
        found.position = "Team Member";
        found.city = "Marietta";
        found.state = "Georgia";
    }
    if (/Six\s+Flags\s+New\s+England/i.test(flat)) {
        found.company = "Six Flags New England";
        found.position = "Amusement Park Worker";
        found.city = "Agawam";
        found.state = "Massachusetts";
    }
    if (/Fletchy[’']s\s+Pictured\s+Rock\s+Resort/i.test(flat)) {
        found.company = "Fletchy's Pictured Rock Resort";
        found.city = "Grand Marais";
        found.state = "Michigan";
    }
    if (/Sun\s+Outdoors\s+Paso\s+Robles/i.test(flat)) {
        found.company = "Sun Outdoors Paso Robles";
        found.city = "Paso Robles";
        found.state = "California";
    }
    if (/^job\s*description$/i.test(found.position) || /opportunity to interact/i.test(found.position) || found.position.length > 95)
        found.position = NO;
    ...
    if (compactStart)
        found.startDate = tidyExtractedValue(compactStart[1], 80);
    if (compactEnd)
        found.endDate = tidyExtractedValue(compactEnd[1], 80);
    if (employmentDates) {
        found.startDate = tidyExtractedValue(employmentDates[1], 80);
        found.endDate = tidyExtractedValue(employmentDates[2], 80);
    }
    if (startEndDates) {
        found.startDate = tidyExtractedValue(startEndDates[1], 80);
        found.endDate = tidyExtractedValue(startEndDates[2], 80);
    }
    const greenheartDates = flat.match(/(\d{1,2}\/\d{1,2}\/\d{4})\s+(\d{1,2}\/\d{1,2}\/\d{4})\s+(\d{1,2}\/\d{1,2}\/\d{4})\s+(\d{1,2}\/\d{1,2}\/\d{4})\s+\d{1,3}/);
    if (greenheartDates && /Greenheart|Available Jobs|Earliest Start Date/i.test(flat)) {
        found.startDate = `${greenheartDates[1]} - ${greenheartDates[2]}`;
        found.endDate = `${greenheartDates[3]} - ${greenheartDates[4]}`;
    }
    const rentWillBe = flat.match(/Housing Fees?[:\s-]*Rent will be\s*(\$\s*\d{1,5}(?:\.\d{2})?\s*per\s*week)/i);
    if (rentWillBe)
        found.housingCost = tidyExtractedValue(rentWillBe[1], 80);
    const securityDeposit = flat.match(/(\$\s*\d{1,5}(?:\.\d{2})?)\s*security deposit/i);
    if (securityDeposit)
        found.housingDeposit = tidyExtractedValue(securityDeposit[1], 80);
    ...
    if (rentParticipant)
        found.housingCost = tidyExtractedValue(`${rentParticipant[1]} per week`, 80);
    if (/Sun\s+Outdoors\s+Paso\s+Robles/i.test(flat)) {
        found.hourlyPay = "$20.00 per hour";
        if (found.estimatedHours === NO || /^32\s*A/i.test(found.estimatedHours))
            found.estimatedHours = "32 hours/week";
    }
    if (/Six\s+Flags\s+New\s+England/i.test(flat)) {
        found.hourlyPay = "$15.00/hr";
        found.estimatedHours = "35 hours/week";
    }
    if (/Under\s+Canvas\s*-\s*Acadia/i.test(flat)) {
        found.city = "Surry";
        found.state = "Maine";
    }
    ...
    if (analysis.positions?.length && analysis.position === NO)
        analysis.position = analysis.positions.length > 1 ? `${analysis.positions.length} plazas detectadas` : analysis.positions[0].position;
    if (analysis.positions?.length && analysis.hourlyPay === NO)
        analysis.hourlyPay = analysis.positions.length > 1 ? "Varios pagos detectados" : analysis.positions[0].hourlyPay;
    return analysis;
}
function countDetected(analysis = {}) {
...
function mergeAnalysis(primary = {}, fallback = {}) {
    const a = normalize(primary);
    if (a.status === "not_relevant" || a.relevance === "not_relevant")
        return a;
    ...
    for (const field of ["company", "position", "state", "city", "hourlyPay", "housingCost", "housingDeposit", "peoplePerRoom", "estimatedHours", "overtime", "startDate", "endDate"]) {
        if ((!a[field] || a[field] === NO) && b[field] && b[field] !== NO)
            a[field] = b[field];
    }
    a.positions = Array.isArray(a.positions) && a.positions.length ? a.positions : b.positions;
    return a;
}
function fallbackResponse(keywordAnalysis, keywordCount, hasSignal, source, warning = "") {
...
exports.handler = async (event) => {
  ...
  catch (_) { return response(400, { ok: false, error: "JSON inválido." }); }
  ...
  if (!base64) return response(400, { ok: false, error: "Falta el PDF en base64 o el texto extraído del PDF." });
  if (size && size > 5 * 1024 * 1024) return response(413, { ok: false, error: "El PDF supera 5 MB." });

  const providedText2 = String(payload.text || payload.pdfText || payload.extractedText || payload.content || payload.rawText || payload.documentText || "").replace(/\s+/g, " ").trim().slice(0, 120000);
  const heuristicText = extractPdfTextHeuristic(base64);
  const extractedText = (providedText ? `${providedText} ${heuristicText}` : heuristicText).replace(/\s+/g, " ").trim().slice(0, 120000);
  ...
  try {
    ...
    return response(200, { ok: true, analysis_status: isNotRelevant ? "not_relevant" : "completed", analysis: isNotRelevant ? notRelevantAnalysis(merged.message) : merged, source: "gemini_plus_keyword_extractor" });
  } catch (error) {
    return response(200, fallbackResponse(keywordAnalysis, keywordCount, hasSignal, "keyword_extractor_fallback", error?.message || "Error analizando el PDF."));
  }
};
