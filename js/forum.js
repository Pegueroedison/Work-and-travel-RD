(() => {
  ...
  async function getMentionFriendSuggestions() {
    ...
    try {
      // V4057: no usamos embedded foreign tables aquí porque Supabase puede marcar
  ...
  async function searchMentionUsers(query = "") {
    ...
    // V4057: primero intenta usar la función RPC inteligente.
    // Si el SQL todavía no está instalado, cae al comportamiento anterior.
    try {
      const { data, error } = await WT.supabase.rpc("search_mention_candidates_v4057", {
        search_text: query,
      ...
      // V4057: si la RPC está instalada pero devuelve vacío, no dejamos la lista muerta.
