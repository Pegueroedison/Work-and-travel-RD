(() => {
  ...
  async function getMentionFriendSuggestions() {
    ...
    try {
      // V4066: no usamos embedded foreign tables aquí porque Supabase puede marcar
  ...
  async function searchMentionUsers(query = "") {
    ...
    // V4066: primero intenta usar la función RPC inteligente.
    // Si el SQL todavía no está instalado, cae al comportamiento anterior.
    try {
      const { data, error } = await WT.supabase.rpc("search_mention_candidates_v4066", {
        search_text: query,
      ...
      // V4066: si la RPC está instalada pero devuelve vacío, no dejamos la lista muerta.
